...
// Adds two three-term ratio questions using Tokyo Disneyland / Tokyo DisneySea tickets.
...
function makeTriple(place, aLabel,aFrac,bLabel,bFrac,cLabel,cFrac,remainingTotal){
  ...
  return {
    type:'triple',place,aLabel,aFrac,bLabel,bFrac,cLabel,cFrac,remainingTotal,
    ab,ac,answerA,answerB,answerC,
    story:`一成、尋子、ダンの3人が${place}へ行き、それぞれ同じ金額のチケット代を払いました。このとき、一成ははじめの所持金の${aLabel}を使いました。尋子ははじめの所持金の${bLabel}を使いました。ダンははじめの所持金の${cLabel}を使いました。また、チケット代を払った後の3人の所持金の合計は${remainingTotal}円になりました。3人のはじめの所持金の比（一成：尋子：ダン）を求めなさい。`
  };
}

function disneyLandTriple(){
  return makeTriple('ディズニーランド','40％',{n:2,d:5},'3/7',{n:3,d:7},'4/9',{n:4,d:9},2205);
}
function disneySeaTriple(){
  return makeTriple('ディズニーシー','25％',{n:1,d:4},'2/9',{n:2,d:9},'3/10',{n:3,d:10},2650);
}

// Replace the growing 15-question set with 6 types x 2 questions.
...
function renderTriple(p){
  ...
  const finalText=`一成：尋子：ダン ＝ ${p.answerA}：${p.answerB}：${p.answerC}`;

  if(step===0){
    return card(`
      <div class="tag">三項比 STEP 1 / 6</div>
      <div class="story">${p.story}</div>
      <div class="stepTitle">まず、三項比のルールを確認しよう</div>
      <div class="prompt"><b>この問題は三項比（さんこうひ）の問題です。三項比（さんこうひ）の問題では、そのまま「内項の積＝外項の積」の公式を</b></div>
  ...
  if(step===1){
    return card(`
      ...
      <div class="story">${p.story}</div>
      <div class="rule"><strong>3人をそのまま扱わず、まず「同じチケット代」に注目します。</strong></div>
      <div class="stepTitle">3人が払ったチケット代の関係はどれ？</div>
      <div class="prompt">${p.aLabel} は ${fracText(p.aFrac)}、${p.bLabel} は ${fracText(p.bFrac)}、${p.cLabel} は ${fracText(p.cFrac)} として考えます。<br><span class="note">※この(1)では、支払い後の合計${p.remainingTotal}円はまだ使いません。</span></div>
      <div class="choices">${choice([
        {t:relation,c:1},
  ...
  if(step===2){
    return card(`
      ...
      <div class="stepTitle">2人ずつの式に分けよう</div>
      <div class="prompt">三項比をいったん<b>二項比2つ</b>に分けます。ここからは、2人ずつなら「内項の積＝外項の積」を利用できます。</div>
      <div class="choices">${choice([
        {t:pairEq,c:1},
  ...
  if(step===3){
    return card(`
      ...
      <div class="rule">2人ずつに分けたので、ここでは<br><strong>「内項の積＝外項の積の公式を利用して」</strong></div>
  ...
  if(step===4){
    return card(`
      ...
      <div class="rule">もう一度、2人だけなので<br><strong>「内項の積＝外項の積の公式を利用して」</strong></div>
  ...
  return card(`
    ...
    <div class="note" style="margin-top:12px">答えの形：${finalText}</div>
  `);
}
...
function checkTriple(a,b,c){
  ...
  if(av===a&&bv===b&&cv===c){
    locked=true;score++;
    feedback(true,`正解！ <strong>一成：尋子：ダン ＝ ${a}：${b}：${c}</strong><br>三項比は、いったん2人ずつの比に分けてから共通する項をそろえます。`);
...
finish=function(){
  bar.style.width='100%';
  app.innerHTML=`<section class="card finish"><h2>ドリル完了！</h2><div class="rule"><strong>二項比：</strong>「内項の積＝外項の積」<br><strong>三項比：</strong>そのまま使わず、まず2人ずつに分ける</div><p style="font-size:20px;font-weight:800">正解数：${score} / ${finalTotalSteps()}</p><p class="note">全12問（6種類×2問）です。「数字を変えてもう一度やる」で、新しい数字の問題に変わります。</p><button class="primary" style="width:100%" onclick="restart()">数字を変えてもう一度やる</button></section>`;
};